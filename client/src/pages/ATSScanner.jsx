            ...
            <div className="max-w-5xl mx-auto relative z-10 pt-32 pb-16 px-6">
                ...
                >
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-secondary/50 w-fit rounded-full border border-border/50 backdrop-blur-md mb-6">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-primary">Resume Comparison Engine</span>
                    </div>
                        ...
                        <div className="bg-card/60 backdrop-blur-3xl border border-border/50 rounded-[40px] p-8 md:p-10 shadow-2xl relative overflow-hidden group">
                            ...
                            <div className="relative z-10 space-y-12">
                                ...
                                <div className="space-y-6">
                                    <h3 className="font-black uppercase tracking-[0.4em] text-[10px] text-primary italic">01 // Step 1: Upload Resume</h3>
                                    <label
                                        htmlFor="resume-upload"
                                        className={`block h-52 border-2 border-dashed rounded-[32px] p-8 flex flex-col items-center justify-center transition-all cursor-pointer relative overflow-hidden group/upload ${file ? 'border-emerald-500/40 bg-emerald-500/5' : 'border-border/50 hover:border-primary/30 hover:bg-secondary/30'}`}
                                    >
                                        <input id="resume-upload" type="file" accept=".pdf,.docx" className="hidden" onChange={handleFileChange} />

                                        <div className="text-center relative z-10">
                                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-5 transition-all duration-500 ${file ? 'bg-emerald-500 text-white scale-110 shadow-lg shadow-emerald-500/20' : 'bg-secondary/50 text-muted-foreground group-hover/upload:scale-110'}`}>
                                                {file ? <FileText size={28} /> : <UploadCloud size={28} />}
                                            </div>
                                            <h4 className={`text-base font-black italic uppercase tracking-tighter mb-1 ${file ? 'text-emerald-500' : 'text-foreground'}`}>
                                                {file ? file.name : "Select Resume"}
                                            </h4>
                                            <p className="text-foreground/20 text-[9px] font-black uppercase tracking-[0.3em]">
                                ...
                                {/* JD Step */}
                                <div className="space-y-6">
                                    <h3 className="font-black uppercase tracking-[0.4em] text-[10px] text-primary italic">02 // Step 2: Job Description</h3>
                                    <textarea
                                        value={jobDescription}
                                        onChange={(e) => setJobDescription(e.target.value)}
                                        placeholder="Paste the job description here..."
                                        className="w-full h-56 bg-secondary/30 border border-border/50 rounded-[32px] p-8 text-foreground text-base placeholder:text-foreground/10 focus:border-primary/40 focus:bg-secondary/50 outline-none transition-all resize-none leading-relaxed font-medium"
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="lg:col-span-4">
                        <div className="bg-card/60 backdrop-blur-3xl border border-border/50 rounded-[40px] p-8 space-y-8 shadow-2xl sticky top-24 overflow-hidden group">
                            ...
                            <div className="relative z-10 space-y-4">
                                <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-foreground/20">System Controls</h3>
                                ...
                                <div className="pt-6 space-y-5">
                                    <div className="flex justify-between items-center px-1">
                                        <span className="text-[9px] font-black text-foreground/20 uppercase tracking-[0.3em]">Analysis Accuracy</span>
                                        <span className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.3em]">AI Verified</span>
                                    </div>
                        ...
                        >
                            ...
                            <div className="relative z-10">
                                <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 border-b border-border/50 pb-12">
                                    <h2 className="text-5xl font-black italic tracking-tighter uppercase leading-none">Your <span className="text-foreground/20">Results</span></h2>
                                </header>
                                    ...
                                    <div className="lg:col-span-4 flex flex-col items-center lg:items-start">
                                        <div className="relative w-64 h-64 flex items-center justify-center mb-10 group/score">
                                            ...
                                            <div className="absolute flex flex-col items-center z-20">
                                                <span className="text-8xl font-black italic tracking-tighter leading-none">{result.score}<span className="text-3xl text-foreground/50">%</span></span>
                                                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-foreground/20 mt-2">Overall Match Score</span>
                                            </div>
                                        </div>
                                        <div className="space-y-4 max-w-sm">
                                            <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-primary italic">AI Feedback</h3>
                                            <p className="text-foreground text-xl font-black italic tracking-tight leading-[1.4] opacity-80">"{result.summary}"</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="lg:col-span-6 space-y-12">
                                        <div className="space-y-6">
                                            <h3 className="text-[10px] font-black uppercase tracking-[0.5em] text-emerald-500 flex items-center gap-4">
                                                <div className="h-[1px] w-8 bg-emerald-500/40" /> Keywords Found
                                            </h3>
                                        ...
                                        <div className="space-y-6">
                                            <h3 className="text-[10px] font-black uppercase tracking-[0.5em] text-yellow-500 flex items-center gap-4">
                                                <div className="h-[1px] w-8 bg-yellow-500/40" /> Missing Keywords
                                            </h3>
                                            <div className="flex flex-wrap gap-3">
                                        ...
                                        <div className="p-10 bg-secondary/30 rounded-[32px] border border-border/50 space-y-5 relative overflow-hidden group/feedback">
                                            <div className="absolute inset-0 bg-primary/5 opacity-0 group-hover/feedback:opacity-100 transition-opacity" />
                                            <h3 className="text-[10px] font-black uppercase tracking-[0.5em] text-primary relative z-10">How to Improve</h3>
                                            <p className="text-foreground/60 text-lg leading-relaxed font-medium relative z-10">
                ...
                <p className="text-center text-[10px] text-foreground/10 mt-24 font-black uppercase tracking-[0.8em] italic opacity-40">Operational Scanning Framework · V7.4</p>
            </div>
        </div>
    );
};
